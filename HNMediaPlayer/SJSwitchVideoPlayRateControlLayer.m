//
//  SJSwitchVideoPlayRateControlLayer.m
//
...
@interface SJSwitchVideoPlayRateControlLayer ()
...
@implementation SJSwitchVideoPlayRateControlLayer
...
- (void)setAssets:(nullable NSArray<SJVideoPlayerURLAsset *> *)assets {
    ...
    [_assets enumerateObjectsUsingBlock:^(HNRateListItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)_refreshItems {
    HNRateListItem *_Nullable cur;
    cur.rate = _videoPlayer.rate;
    if ( cur != nil ) {
        HNRateListItem *selected = cur;
        for ( SJEdgeControlButtonItem *item in self.items ) {
            HNRateListItem *asset = self.assets[item.tag];
            item.title = [NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
                make.append([NSString stringWithFormat:@"x%.02f", asset.rate]);
